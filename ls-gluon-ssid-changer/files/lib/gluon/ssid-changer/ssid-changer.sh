...
then
	...
	for HOSTAPD in $(ls /var/run/hostapd-phy*); do # check status for all physical devices
		...
		then
			logger -s -t "ls-gluon-offline-ssid" -p 5 "TQ is $GATEWAY_TQ, SSID is $CURRENT_SSID, change to $ONLINE_SSID" # write info to syslog
...
then
	...
	if [ $(expr $(date "+%s") / 60 % $MINUTES) -eq 0 ]; then
		for HOSTAPD in $(ls /var/run/hostapd-phy*); do # check status for all physical devices
			...
			if [ "$CURRENT_SSID" == "$ONLINE_SSID" ]
			then
				logger -s -t "ls-gluon-offline-ssid" -p 5 "TQ is $GATEWAY_TQ, SSID is $CURRENT_SSID, change to $OFFLINE_SSID"
